...
<html>
    ...
    <input type="text" id="cont" value="${ll[0] }" style="display: none;" >
        <div class="x-nav">
            ...
            <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"   id="ddi" href="stayStill/stayStill-list" title="刷新"><i class="layui-icon" style="line-height:30px">ဂ</i></a>
        </div>
        <div class="x-body">
            <form class="layui-form x-center" action="stayStill/stayStill-mh" style="width:800px">
                <div class="layui-form-pane" style="margin-top: 15px;">
                  ...
                  <div class="layui-input-inline">
                    ...
                    <div class="layui-input-inline">
                      <input class="layui-input" placeholder="借款标题" name="title" id="title" value="${ll[3] }">
                    </div>
<!--                     <div class="layui-input-inline"> -->
...
<!--                     </div> -->
                ...
                <c:forEach items="${list }" var="list">
                    <tr>
                        ...
                        <td >
                         ...
                         <td >
 							<fmt:formatDate value="${list.lendTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
                        </td>
                        <td >
                            ${list.type }
                        </td>
                        ...
                        <td >
 							<fmt:formatDate value="${list.stillTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
                        </td>
                        <td>
                            ${list.stayGold }
                        </td>
                        ...
                        <c:if test="${list.staystatus=='未偿还'}">
                        <button class="layui-btn layui-btn-small" onclick="getonckli('${list.id}');">未偿还 </button>
                        
                        </c:if>
                          <c:if test="${list.staystatus=='已偿还'}">
                        <button  class="layui-btn layui-btn-small layui-btn-disabled" lay-filter="add" lay-submit="" disabled="disabled">已偿还</button>
                        </c:if>  
                        </td>
                    </tr>
                    </c:forEach>
     ...
     function getonckli(id){
    	 window.location.href='stayStill/stayStill-upda?id='+id;
 		
     }
        
		        function option(curr){
		        	var title=document.getElementById("username").value;
		        	var people=document.getElementById("title").value;
		        	window.location.href='stayStill/stayStill-fy?id='+curr+'&'+'userName='+title+'&'+'title='+people;
		
		        }

 function exportt(){
		      		var username=document.getElementById("username").value;
				    var time=document.getElementById("title").value;
		        	var cont=document.getElementById("cont").value;
		        	if(parseInt(cont)>parseInt(0)){
		        		layer.confirm('确定要导出吗？',function(index){
	        		     $.ajax({  
	  		                url: 'stayStill/stayStill-testsExport',  
	  		                type: 'POST', 
	  		                data: {
	  		                	userName:username,
	  		                	title:time,
